window.addEventListener("load", function (){
    ...
    if(admin_menu){
        admin_menu.addEventListener("mouseleave", function (){
            document.querySelector(".admin_menu").style.transform="translate(calc(-100% + 30px), -50%)";
        });
    }
    const admin_menu_btns = document.querySelector(".admin_menu button");
    if(admin_menu_btns){
        admin_menu_btns.addEventListener("click", function (){
            if(document.querySelector(".admin_menu").style.transform == "translate(0px, -50%)"){
                document.querySelector(".admin_menu").style.transform="translate(calc(-100% + 30px), -50%)";
            }
            else{
                document.querySelector(".admin_menu").style.transform="translate(0, -50%)";
            }
        });
    }
